...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($full_name) || empty($email)) {
        ...
        if ($stmt_update) {
            ...
            echo "<script>alert('Gagal menyiapkan statement UPDATE.');</script>";
        }
    }
}
...
if ($stmt_fetch) {
    $stmt_fetch->execute([$lecturer_user_id]);
    if ($row = $stmt_fetch->fetch()) {
    ...
    echo "<script>alert('Gagal mengambil data profil.');</script>";
}
